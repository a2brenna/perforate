...
int main(int argc, char *argv[]){
    ...
    const auto client_handler = [&index, &tags](const int &client_fd){
        ...
        const auto w = write(client_fd, response.c_str(), response.size());
        (void)w;
        close(client_fd);
    ...
    while(true){
        const auto client_fd = accept4(incoming_fd, nullptr, nullptr, 0);
        if(client_fd < 0){
